...
export default function CourseDetails() {
  ...
  if (!course) {
    return <div className="max-w-4xl mx-auto px-6 py-12" style={{ color: 'var(--color-text-muted)' }}>Loading...</div>;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 md:px-10 py-12">
      <Link to="/" className="text-sm font-medium mb-6 inline-block" style={{ color: 'var(--color-text-muted)' }}>
        ← Back to courses
      </Link>
      ...
      <div className="flex items-center gap-4 mb-8 text-sm" style={{ color: 'var(--color-text-muted)' }}>
      ...
      >
        ...
        <div className="flex items-center justify-between">
          ...
          {user?.role === 'student' && !enrolled && (
            ...
            >
              {enrolling ? 'Enrolling...' : course.price == 0 ? 'Enroll for free' : 'Buy course'}
            </button>
          )}
        ...
        {message && (
          <p className="mt-4 text-sm font-medium" style={{ color: 'var(--color-primary)' }}>{message}</p>
        )}
      </div>
      ...
      <div className="space-y-2 mb-10">
        {course.lessons?.length === 0 && (
          <p style={{ color: 'var(--color-text-muted)' }}>No lessons added yet.</p>
        )}
        {course.lessons?.map((lesson, i) => (
          ...
          >
            <span className="font-display font-semibold text-sm w-6" style={{ color: 'var(--color-text-muted)' }}>
              {i + 1}
            </span>
      ...
      <div className="space-y-4">
        ...
        {reviews.map((r) => (
          <div key={r.id} className="rounded-xl border p-4" style={{ borderColor: 'var(--color-border)' }}>
            <div className="flex items-center justify-between mb-1">
              <span className="font-semibold" style={{ color: 'var(--color-text)' }}>{r.student?.name}</span>
